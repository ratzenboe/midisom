...
class MidiSom(object):
    ...
    def plot_distance_map(self, df_data,
            target=None, n_samples=0, figsize=(12,10), outfile=None,
            markers=['o','s','v','p','*'], colors=['r','y','b','darkviolet','m']):
        plt.figure(figsize=figsize)
        plt.pcolor(self.distance_map().T, cmap='bone_r')
        plt.colorbar(shrink=0.9)
        n_samples = df_data.shape[0] if (n_samples>=df_data.shape[0] or n_samples==-1) else n_samples
        if n_samples>0 and target is not None:
            # consistency check
            n_unique_target = np.unique(target).shape[0]
            if (len(markers) < n_unique_target) or (len(colors) < n_unique_target):
                raise ValueError('Too little markers or colors provided to account for {} '\
                        'target values'.format(n_unique_target))
            for cnt in tqdm(range(n_samples)):
                w = self.winner(df_data.iloc[cnt].values) # getting the winner
                # palce a marker on the winning position for the sample xx
                plt.plot(w[0]+.5, w[1]+.5, markers[target[cnt]], markerfacecolor='None',
                         markeredgecolor=colors[target[cnt]], markersize=17, markeredgewidth=2)
    ...
    def plot_activation_response(self, df_data, target,
        ...
        plt.colorbar(shrink=0.9)
        # plot winning samples if specified
        n_samples = df_data.shape[0] if (n_samples>=df_data.shape[0] or n_samples==-1) else n_samples
        if n_samples>0 and target is not None:
            # consistency check
            n_unique_target = np.unique(target).shape[0]
    ...
    def plot_label_map(self, df_data, labels, figsize=(10,10), outfile=None):
        ...
        for position in tqdm(labels_map.keys()):
            label_fracs = [labels_map[position][l] for l in label_names]
            plt.subplot(the_grid[shift-position[1], position[0]], aspect=1)
            patches, texts = plt.pie(label_fracs)
        leg = plt.legend(patches, label_names, prop={'size': 13},
                         loc='center right',
                         bbox_to_anchor=(shift+1, shift/2.))
        #plt.tight_layout()
        if isinstance(outfile, str):
            plt.savefig(outfile, bbox_extra_artists=(leg,), bbox_inches='tight')

    def plot_most_imp_feat(self, df_data, figsize=(8,8), markersize=17, outfile=None):
        from matplotlib.patches import Patch
        Z = np.zeros((self._weights.shape[0], self._weights.shape[1]))
        plt.figure(figsize=figsize)
        for i in tqdm(np.arange(self._weights.shape[0])):
            for j in np.arange(self._weights.shape[1]):
                feature = np.argmax(self.get_weights()[i, j , :])
                plt.plot([j+.5], [i+.5], 'o', color=plt.cm.get_cmap("jet",
                    df_data.shape[-1])(np.arange(df_data.shape[-1]))[feature].tolist(),
                    marker='s', markersize=markersize)

        legend_elements = [Patch(facecolor=plt.cm.get_cmap("jet",
            df_data.shape[-1])(np.arange(df_data.shape[-1]))[i].tolist(),
            edgecolor='w', label=f) for i, f in enumerate(list(df_data.columns))]
        ...
        plt.tick_params(axis='both', which='both',
                        bottom=False, top=False, labelbottom=False,
                        left=False, right=False, labelleft=False)
        plt.xlim([0, self._weights.shape[0]])
        plt.ylim([0, self._weights.shape[1]])
        #plt.tight_layout()
        if isinstance(outfile, str):
            plt.savefig(outfile, bbox_extra_artists=(leg,), bbox_inches='tight')
...
class TestMidisom(unittest.TestCase):
    ...
    def test_unavailable_neigh_function(self):
        with self.assertRaises(ValueError):
            MidiSom(5, 5, 1, neighborhood_function='boooom')
    ...
    def test_random_seed(self):
        som1 = MidiSom(5, 5, 2, sigma=1.0, learning_rate=0.5, random_seed=1)
        som2 = MidiSom(5, 5, 2, sigma=1.0, learning_rate=0.5, random_seed=1)
        # same initialization
    ...
    def test_train_batch(self):
        som = MidiSom(5, 5, 2, sigma=1.0, learning_rate=0.5, random_seed=1)
        data = array([[4, 2], [3, 1]])
    ...
    def test_random_weights_init(self):
        som = MidiSom(2, 2, 2, random_seed=1)
